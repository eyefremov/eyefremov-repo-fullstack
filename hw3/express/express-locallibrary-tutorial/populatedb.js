...
console.log(
  'This script populates some test books, authors, genres, and book instances to your database. Specified database as argument - e.g.: node populatedb "mongodb+srv://cooluser:coolpassword@cluster0.cojoign.mongodb.net/local_library?retryWrites=true&w=majority&appName=Cluster0"',
);
...
async function authorCreate(index, firstName, familyName, dateOfBirth, dateOfDeath) {
  ...
  if (dateOfBirth) authorDetail.date_of_birth = dateOfBirth;
  if (dateOfDeath) authorDetail.date_of_death = dateOfDeath;
...
async function bookInstanceCreate(index, book, imprint, dueBack, status) {
  ...
  if (dueBack) bookInstanceDetail.due_back = dueBack;
